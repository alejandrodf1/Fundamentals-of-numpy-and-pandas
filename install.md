# Numpy and pandas fundamentals guide installation
...
activate numpy_and_pandas_fundamentals
```
...
To move beyond notebook prototyping, all reusable code should go into the `numpy_pandas/` folder package. To use that package inside your project, install the project's module in editable mode, so you can edit files in the `numpy_pandas` folder and use the modules inside your notebooks :
...
```python
from numpy_pandas.utils.paths import data_dir
data_dir()
```
